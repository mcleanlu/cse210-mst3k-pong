...
from game.casting.silhouette import Silhouette
...
from game.scripting.draw_silhouette_action import DrawSilhouetteAction
...
class SceneManager:
    ...
    VIDEO_SERVICE = RaylibVideoService(GAME_TITLE, SCREEN_WIDTH, SCREEN_HEIGHT)
    ...
    DRAW_SILHOUETTE_ACTION = DrawSilhouetteAction(VIDEO_SERVICE)
    ...
    def _prepare_next_level(self, cast, script):
        ...
        self._add_racket(cast)
        self._add_silhouette(cast)
        self._add_dialog(cast, MOVIE_SIGN_START)
        script.clear_actions(INPUT)
        script.add_action(INPUT, TimedChangeSceneAction(IN_PLAY, 2))
        self._add_output_script(script)
        script.add_action(OUTPUT, PlaySoundAction(self.AUDIO_SERVICE, START_SOUND))
    ...
    def _add_silhouette(self, cast):
        cast.clear_actors(SILHOUETTE_GROUP)
        x = CENTER_X - (SILHOUETTE_WIDTH / 2)
        y = SCREEN_HEIGHT - SILHOUETTE_HEIGHT
        position = Point(x, y)
        size = Point(SILHOUETTE_WIDTH, SILHOUETTE_HEIGHT)
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        image = Image(SILHOUETTE_IMAGE)
        silhouette = Silhouette(body, image, True)
        cast.add_actor(SILHOUETTE_GROUP, silhouette)
    ...
    def _add_racket(self, cast):
        ...
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(RACKET_IMAGES["left"], RACKET_RATE)
        score = 1
        racket2 = Racket(body, animation, score)
        ...
        velocity = Point(0, 0)
        body = Body(position, size, velocity)
        animation = Animation(RACKET_IMAGES["right"], RACKET_RATE)
        score= 1
        racket1 = Racket(body, animation, score)
    ...
    def _add_output_script(self, script):
        ...
        script.add_action(OUTPUT, self.DRAW_SILHOUETTE_ACTION)
